...
### José Andrés Lorca - 29/6/2026, 14:55:55
...
### José Andrés Lorca - 8/7/2026, 15:37:58

Acting as PO Proxy to help accelerate the estimation flow, following Ely's guidance, [https://jira.upexgalaxy.com/browse/BK-49#icft=BK-49](https://jira.upexgalaxy.com/browse/BK-49#icft=BK-49) is estimated at 5 SP under a bounded MVP scope.
...
`public.activity*log` already exists (`supabase/migrations/0009*cross*cutting.sql`) with `workspace*id, actor*user*id, entity*type, entity*id, action, payload jsonb, created*at`, an index on `(workspace*id, created_at desc)`, and real writers already populating it (module mutations, [https://jira.upexgalaxy.com/browse/BK-59#icft=BK-59](https://jira.upexgalaxy.com/browse/BK-59#icft=BK-59)). This is not a from-scratch design.
...
- New RPC `bunkai*list*activity(p*actor*user*id, p*workspace*id, p*limit, p*cursor*created*at, p*cursor*id)`, `SECURITY DEFINER`, with the actor-bind guard (`auth.uid() = p*actor*user*id`) baked in from the start — [https://jira.upexgalaxy.com/browse/BK-37#icft=BK-37](https://jira.upexgalaxy.com/browse/BK-37#icft=BK-37) shipped without this and had to retrofit it in a follow-up migration after a caller-impersonation gap was found; [https://jira.upexgalaxy.com/browse/BK-38#icft=BK-38](https://jira.upexgalaxy.com/browse/BK-38#icft=BK-38) baked it in from day one citing that exact incident. No reason for [https://jira.upexgalaxy.com/browse/BK-49#icft=BK-49](https://jira.upexgalaxy.com/browse/BK-49#icft=BK-49) to repeat the mistake.
...
- Server component (wherever the feed is surfaced) calls the SAME RPC directly on first read, not its own `/api/v1/activity` route — established, unbroken pattern across `/settings/account`, `/settings/workspaces` ([https://jira.upexgalaxy.com/browse/BK-89#icft=BK-89](https://jira.upexgalaxy.com/browse/BK-89#icft=BK-89)), and the Run History screen ([https://jira.upexgalaxy.com/browse/BK-37#icft=BK-37](https://jira.upexgalaxy.com/browse/BK-37#icft=BK-37)): avoids a self-referential HTTP hop. The API route exists for client-side "load older" pagination and Bearer-PAT callers.
...
2. The new RPC adds its own explicit `workspace_id` filter and the actor-bind guard on top of RLS — same "RLS is defense-in-depth, the RPC predicate is the real enforcement for this report" reasoning `0041`'s own migration comment states for [https://jira.upexgalaxy.com/browse/BK-38#icft=BK-38](https://jira.upexgalaxy.com/browse/BK-38#icft=BK-38).
...
Drop the `SECURITY DEFINER` + `p*actor*user_id` shape entirely. It was reaching for [https://jira.upexgalaxy.com/browse/BK-37#icft=BK-37](https://jira.upexgalaxy.com/browse/BK-37#icft=BK-37)/BK-38's pattern without asking whether this feature needs their privilege escalation — it doesn't (no cross-table aggregation, no transactional-integrity need). `ADR-0001` already settled this fork for exactly this case: "(A) SECURITY DEFINER RPCs with an explicit actor... (B) give the PAT caller an RLS-scoped client... Path B was chosen." A read-only feed is Path B territory.
...
- ***New migration needed***: `activity*log*workspace*created*at*id*idx (workspace*id, created*at desc, id desc)`. The existing index (`workspace*id, created*at desc`) has no `id` tie-break column and does not serve the keyset seek — same gap [https://jira.upexgalaxy.com/browse/BK-37#icft=BK-37](https://jira.upexgalaxy.com/browse/BK-37#icft=BK-37) hit and fixed with a purpose-built index; this proposal's first draft claimed no schema change was needed, which was wrong.
- Route: `GET /api/v1/activity`, no workspace id in the path — reusing the caller's active-workspace resolution. ***Known dependency***: [https://jira.upexgalaxy.com/browse/BK-182#icft=BK-182](https://jira.upexgalaxy.com/browse/BK-182#icft=BK-182) (open bug) — Bearer-PAT active-workspace resolution is currently broken for at least one existing route. Link [https://jira.upexgalaxy.com/browse/BK-49#icft=BK-49](https://jira.upexgalaxy.com/browse/BK-49#icft=BK-49) to BK-182; this new route inherits that gap until it's fixed.

***3. Event allowlist (new — the taxonomy question, previously undecided by omission)***

The RPC needs a `p*actions text[] default null` filter and an explicit MVP allowlist, not "return every row in the table." Concretely: [https://jira.upexgalaxy.com/browse/BK-35#icft=BK-35](https://jira.upexgalaxy.com/browse/BK-35#icft=BK-35) (`run*step.marked`) fires once per step per run and would drown every other event type in an unfiltered feed within a day of use. Proposed MVP allowlist: the entity-create/update/complete-level events (`module.*`, `atc.created`, `test.created`, `run.finished`, `run.aborted`) — excluding step-level noise (`run_step.marked`) and no-op/internal events. `.context/business/events.md` needs a refresh in the same story — it currently documents 6 of the (at least) 13 actions now writing to the table.
...
This is a deliberate departure from `master-design-plan.md` §4.2 (which maps [https://jira.upexgalaxy.com/browse/BK-49#icft=BK-49](https://jira.upexgalaxy.com/browse/BK-49#icft=BK-49) into the Home dashboard's `home.jsx` mockup) — per this repo's design-fidelity rule, it gets ratified as a `master-design-plan.md` §5 divergence entry as part of Stage 1 for this story (not silently built).

Separately: the product owner is evaluating whether to seed a proper "Home Dashboard" epic via `/product-management` now that 2 of its 3 feeder stories ([https://jira.upexgalaxy.com/browse/BK-8#icft=BK-8](https://jira.upexgalaxy.com/browse/BK-8#icft=BK-8), done; [https://jira.upexgalaxy.com/browse/BK-46#icft=BK-46](https://jira.upexgalaxy.com/browse/BK-46#icft=BK-46), in flight) are ready — that's a distinct initiative, not a blocker for [https://jira.upexgalaxy.com/browse/BK-49#icft=BK-49](https://jira.upexgalaxy.com/browse/BK-49#icft=BK-49), tracked outside this ticket.

[https://jira.upexgalaxy.com/browse/BK-49#icft=BK-49](https://jira.upexgalaxy.com/browse/BK-49#icft=BK-49) is now fully unblocked. Starting Stage 1 implementation planning.

---

### Ely - 31/7/2026, 14:41:12

## Chain strategy decision (Stage 1 → Stage 2 gate, risk=High per the Workload Forecast)

***Chain strategy:**** ****feature-branch-chain***

***Decision trace****: Q1=No (new domain logic — a new RPC pair following ADR-0001 Path B, a new PII-exposure ADR, and a wholly new UI surface with zero existing components to reuse, unlike [https://jira.upexgalaxy.com/browse/BK-89#icft=BK-89](https://jira.upexgalaxy.com/browse/BK-89#icft=BK-89) which reused BK-87's `WorkspacesList`; not a rename/formatter/generated-code/vendor-update) · Q2=No (the natural DB/API/UI split has API alone — `route.ts` + `response.ts` + the full `ActivityItemSchema` discriminated-union-across-8-event-types + `route.test.ts` — realistically landing 450-650 lines, and UI alone — new page + all-new list/skeleton/empty/row components with actor-badge and item-label rendering + tests, nothing reused — realistically landing 700-1000+ lines; neither clears the <400-line-per-slice bar this leaf requires) · Q3=Yes (the new RPC's `jsonb {items, next*cursor`} shape and the generated TS types for `bunkai*list_activity`/the actor-resolution helper are shared scaffolding the API slice defines and the UI slice directly imports and calls — per this codebase's own established pattern of the server component calling the same RPC/query the route uses; a partial merge of the UI slice without the API slice would not compile) → ****feature-branch-chain***
...
Same mechanics as this repo's own precedent for a comparably-sized new feature ([https://jira.upexgalaxy.com/browse/BK-35#icft=BK-35](https://jira.upexgalaxy.com/browse/BK-35#icft=BK-35), `feat/BK-35-mark-run-step`, DB/API/Realtime/UI on one long-lived branch).

---

### Automation for Jira - 31/7/2026, 15:02:25

🔎 Pull Request created. Task is pending to ANALYZE and REVIEW by the team. Waiting for PR Approval.

---

### Automation for Jira - 31/7/2026, 15:02:33

✅ Pull Request is successfully MERGED. Task is Done.

---

### Ely - 31/7/2026, 23:50:13

## QA handoff — merged to staging

***PR***: [https://github.com/upex-galaxy/upex-bunkai-tms/pull/83](https://github.com/upex-galaxy/upex-bunkai-tms/pull/83) (merge commit `66ba0a1`), assembled from the full chain: DB (#75), API (#77), UI (#79), gap-closure (#80), security fix (#82).

***What shipped***: standalone `/activity` route (§5 D15 — not embedded in Home, which is 0% built). Read-only, paginated, newest-first workspace activity feed over the existing `activity_log` table. Nav entry added to the sidebar (Activity, after Home).

***Please re-check on staging (a real security fix landed in this chain, not just a feature)***:

1. Actor emails render correctly on feed rows for entries you wrote yourself and for a co-member's entries.
2. ***As two different workspace members*** (or with a second test account), confirm neither can see the other workspace's activity feed at all — this route's cross-workspace isolation depends on RLS working correctly end to end, verified in code/DB tests but never in a live browser (see note below).
3. Load-older pagination at the bottom of the feed — confirm no duplicate or skipped rows across a page boundary.
4. Empty state (a workspace with no activity) reads clearly, distinct from an error state.
5. `run.aborted` entries (if any exist / once BK-35's abort flow produces one) must NEVER show the operator's free-text abort reason — only "aborted a run", no reason text anywhere in the DOM or network response.

***Known gap, flagged not silently absorbed***: `avalanche-2026-07`'s §9b suspended mandatory live-UI/browser validation for this whole batch run (throughput tradeoff, ratified by the repo owner) — this story never got an actual browser-driven Stage 3 pass, only code-level + DB-integration-level verification. Item 2 above is the single highest-value thing for a human to actually click through, since it's the one property no test in this chain could observe end-to-end in a real rendered page.

***Also worth knowing***: mid-chain, a full-diff adversarial review caught a real, live, exploitable cross-tenant email-disclosure bug in the actor-resolution RPC (fixed same-day, PR #82, already applied to the live DB before this PR merged — not something staging QA needs to separately verify, just flagging why the merge took longer than the other tickets this run).

Full technical detail: `.session/sprint-development-queue/avalanche-2026-07/escalation-log.md` (local, not in this repo's git history).
