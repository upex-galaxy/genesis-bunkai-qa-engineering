...
### Ely - 6/8/2026, 19:31:48

## Delivery handoff — BK-211 merged to `staging`

***Merged**** via PR #137 (`feature/BK-211-run-terminal-notifications`), merge commit `10c8bbe`, ****ancestor-verified*** against `origin/staging`. Status auto-transitioned to `Ready For QA`.

### Schema applied

Both migrations are live on `fmbpikzpkafptqximhxn`:

| Migration | Class | What it does |
| --- | --- | --- |
| `0066*run*event*notifications` | additive | `bunkai*notify*run*event()` + `AFTER INSERT ON activity*log` trigger, gated on `entity*type = 'run'` and `action in ('run.finished','run.aborted')` |
| `0067*run*finish*abort*via` | rewrite | `DROP` + recreate of `bunkai*finish*run` and `bunkai*abort*run`, adding `p_via text default null` |

Live definitions re-read and verified after apply: exactly one overload of each function remains (`p*actor*user*id uuid, p*run*id uuid, p*verdict|p*reason text, p*via text`), both still `SECURITY DEFINER`. The `DROP` in `0067` was load-bearing rather than cleanup — PostgREST calls `.rpc()` with named parameters, so a 3-argument call would have been an exact match against the old never-notifying body and would have silently defeated the whole migration.

### Verification

- `lib/notifications/run-event-trigger-isolation.test.ts` — ***7 pass / 0 fail***. These are the assertions that exercise the real production write path: every mutation goes through the actual `bunkai*finish*run` / `bunkai*abort*run` RPCs rather than a hand-inserted `activity_log` row. They were skipping by design until the schema landed.
- `lib/notifications` + `lib/supabase` suites — ***76 pass / 0 fail***, re-run after merging `staging` into the branch.
- Review adjudication: ***0 BLOCKER / 0 MAJOR / 0 MINOR / 0 NIT*** unresolved.

### Behaviour QA should exercise

The suppression rule is the part worth probing, because identity alone is deliberately **not** the predicate (ruling `12196`, superseding `12173`):

- Elena starts a run and finishes it herself ***in the browser**** -> ****no*** notification, badge unchanged (AC Scenario 5)
- Elena starts a run and an agent/PAT finishes it — ***even authenticating as Elena**** -> she ****is*** notified (AC Scenario 1). Under ADR-0001 Path B a PAT impersonates its owning user, so an identity-only check would have silently broken this case
- Teammate finishes or aborts Elena's run -> notified, with the verdict, and the abort reason on abort (Scenarios 2, 3)
- Panel open while an agent finishes -> row arrives in real time (Scenario 4)
- A run whose starter's account was deleted (`executor*user*id` is `ON DELETE SET NULL`) -> no notification, silently

A merge conflict against `staging` was resolved during merge; both conflicting files were sync-owned PBI cache files and were regenerated from live Jira rather than hand-merged.
