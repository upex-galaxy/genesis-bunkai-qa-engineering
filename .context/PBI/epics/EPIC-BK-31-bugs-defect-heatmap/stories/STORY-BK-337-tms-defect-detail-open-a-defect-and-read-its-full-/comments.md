...
### Ely - 18/8/2026, 21:56:24

## QA Report — BK-337 — PASSED

***Verdict******:****** GO.*** All 22 Test Runs in [ATR BK-518] PASSED. Zero defects found — every scenario in Acceptance Criteria Revision 2 behaves exactly as specified.

***Modality******:****** Jira-xray.**** ATP = [BK-516], ATR = [BK-518], both parented under the new ****QA Master Test Plan**** (BK-514) / ****QA Test Artifacts*** (BK-515) process epics. 22 Gherkin Tests (BK-519..BK-540), each linked "is designed by" ATP and "is executed by" ATR, direction-verified against the acli `--out`/`--in` inversion gotcha.

### Coverage

18 acceptance scenarios (AC1.1-1.4, AC2.1, AC3.1-3.4, AC4.1, AC5.1-5.3, E-1..E-6) + 2 API-contract checks + 1 notification-resolver check (code-verified, no live fixture) + 1 risk-beyond-AC addition (status/severity chip matrix). Full precondition state matrix seeded directly in the staging DB for this run: run-linked defect, standalone defect, evidence at 0/6/10 (incl. a legacy `javascript:` entry), an archived-module defect, and three role/tenant fixtures (foreign workspace, admin-role, viewer-role membership) — no new Supabase Auth accounts created, avoiding the known email rate-limit.

### Notable confirmations

- The trickiest arithmetic in the ticket — Origin panel "Failed at step N" from a 0-based `run_steps.position` — renders correctly (position 2 → "step 3").
- Evidence scheme security (AC3.4) holds: the `javascript:` entry has no `href` and no link role in the DOM; the 9 http/https entries all carry `target="_blank" rel="noopener noreferrer"`.
- E-3 (wrong project slug) does not leak the record — it surfaces a generic "not available" state rather than the bug content.
- Deliberately-cut items (Expected/Actual, layer/environment rows, in-list step highlight, invented identifiers) are absent as expected — not re-raised as gaps.

### Two non-blocking notes for the team (not defects, not filed)

1. ***Reporter resolution caveat***: on the two brand-new role-fixture workspaces, "Filed by" shows "Unknown" for a user id that resolves correctly on the long-lived main workspace. Traced to this session's fixtures being seeded directly via SQL rather than through the app's own workspace-onboarding flow — a test-data artifact, not app behaviour.
2. ***Active-workspace-scoped routing discovery****: `/projects/[slug]/...` resolves against the browser session's **currently active* workspace, not a global search across every workspace the signed-in user belongs to. Switching via the top-left workspace picker was required to reach the admin-role and viewer-role fixtures. Worth a line in onboarding docs for anyone building multi-workspace QA fixtures the way this session did.

Full per-scenario evidence (screenshots + DOM attribute checks) is attached to each Test Run in [BK-518].
