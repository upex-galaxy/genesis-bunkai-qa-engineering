...
### pinto.lucas.nahuel - 13/8/2026, 20:04:37
...
### pinto.lucas.nahuel - 13/8/2026, 21:50:12

@@Ely Ready For Dev

---

### Ely - 16/8/2026, 15:30:55
...
### Automation for Jira - 16/8/2026, 16:52:48

🔎 Pull Request created. Task is pending to ANALYZE and REVIEW by the team. Waiting for PR Approval.

---

### Automation for Jira - 16/8/2026, 17:24:07
...
### Ely - 16/8/2026, 17:25:45

## Ready For QA — Billing overview merged to staging

PR: https://github.com/upex-galaxy/upex-bunkai-tms/pull/176 (merge commit `18662df6`)

Implements the plan/seats/usage overview exactly per the binding technical contract in comments `12414`-`12419`: `lib/billing/plan-tiers.ts` (tier ladder as TS constants), migration `0072*workspace*billing_overview.sql` (additive `SECURITY INVOKER` RPC, no actor param, step-0 admin gate), `GET /api/v1/workspaces/{id}/billing` (404 non-disclosure, never 403), and the Settings > Billing screen (nav item now live, plan card, seat meter, usage meters, inert upgrade entry with a `soon` tag).

Stage 3 independent review: 1 MAJOR + 3 MINOR + 2 NIT, all fixed (detail in `.context/PBI/epics/EPIC-BK-224-billing-plans/stories/STORY-BK-229-billing-view-my-workspace-plan-seats-and-usage/review.md` on the branch). Test suite: 1547/1547 passing repo-wide, including a DB-integration test (`lib/billing/billing-overview-isolation.test.ts`) against the real database through real authenticated sessions — owner/admin see true counts, a member gets `null`, a foreign or unknown workspace gets `null`.

Live-UI validated on the real dev server against the real database (screenshot + network trace) before merge.

@pinto.lucas.nahuel — assigning to you as this story's shift-left QA owner. The `shift-left-reviewed` label's ATP (18 outlines) should apply directly; `/sprint-testing` should detect it and shorten phases 1-3 per your own 2026-08-13 note on this ticket.

One disclosed scope note carried in the PR: the Cloud tier's plan card shows no renewal-date row (Community shows "No active subscription", Enterprise shows "Per contract" — both static tier metadata). No subscription/renewal schema exists in the database yet; BK-231 is the natural home for a real renewal date. Not a silent gap — flagged in the PR body under "Known scope note".
